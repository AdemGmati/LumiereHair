...
const navigation = [
  ["Home", "/"],
  ["Shop All", "/products"],
  ["Clip-Ins", "/products?cat=clip-ins"],
  ["Ponytails", "/products?cat=ponytails"],
  ["Tape", "/products?cat=tape"],
  ["New", "/products?badge=new"],
  ["Sale", "/products?badge=sale"],
] as const;
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-[#ebe6f1] bg-white/92 backdrop-blur">
        <div className="mx-auto grid min-h-[72px] max-w-[1200px] grid-cols-[1fr_auto] items-center gap-4 px-5 md:grid-cols-[1fr_auto_1fr]">
          ...
          <label className="relative hidden w-[min(420px,100%)] md:block">
            <Search className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#796782]" />
            <input
              type="search"
              value={search}
              ...
              aria-label="Search"
              className="w-full rounded-full border border-[#ebe6f1] bg-[#f8f8fc] py-2.5 pr-4 pl-10 text-sm outline-none focus:border-violet-400"
            />
          </label>
          ...
          <div className="flex items-center justify-end gap-1">
            ...
            >
              ...
              {itemCount > 0 && (
                <span className="absolute right-0 top-0 grid min-w-[18px] place-items-center rounded-full bg-violet-500 px-1 text-[10px] text-white">
                  {itemCount}
                </span>
              )}
            </button>
          ...
          <div className="mx-auto flex max-w-[1200px] justify-center overflow-auto px-5">
            {navigation.map(([label, href]) => (
              <Link
                key={label}
                href={href}
                className={`border-b-2 px-4 py-3 text-[13px] font-medium uppercase tracking-[.04em] ${pathname === "/products" && label === "Shop All"
      ...
      {/* Mobile Drawer */}
      <div
        className={`fixed inset-0 z-50 flex flex-col gap-1 bg-white p-5 transition-transform md:hidden ${menuOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
